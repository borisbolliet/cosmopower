...
np.savetxt(path_to_repo+'k_modes.txt', k)

cosmo = classy.Class()

params_lhs = np.load(path_to_repo+'your_LHS_parameter_file_200522.npz')
...
def spectra_generation(i):
    ...
    try:
        ...
        f=open(path_to_repo+'linear.dat','ab')
        ...
        f=open(path_to_repo+'boost.dat','ab')
        np.savetxt(f, [cosmo_array])
        f.close()
